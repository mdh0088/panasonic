...
if(location.href.indexOf("product-detail") > -1){
    document.addEventListener('DOMContentLoaded', sendCertList);
    document.addEventListener('click', sendCertList);
}
function sendCertList(){
	var container = document.querySelectorAll('.txt dl');
	var inner = '<a href="https://pessda.panasonic.co.kr/cert/list.php?cate=6" target="_blank"><i><img src="/img/summary_icon05.png" alt=""></i>인증서</a>';
	for(var i = 0; i < container.length; i++){
		if(container[i].querySelector("dd").textContent == "정보통신"){
			var cont = container[i].parentElement;
			var certEl = Array.prototype.slice.call(cont.querySelectorAll("a")).filter(function(el){
				return el.textContent === '인증서';
			})[0];
			if(certEl === undefined){
				cont.querySelector('.btn').innerHTML += inner;
			}
		}
	}
}
