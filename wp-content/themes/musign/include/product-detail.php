			...
			if (kmew_chk!='KMEW') {	
    			...
    			if (chk_length<10) {
        			...
        			if (lang=='en') {
        				...
        				if (map_1_chk.indexOf('.dwg')!=-1 || map_1_chk.indexOf('.DWG')!=-1) {
                			//inner += "<a href='/img/product_map/"+result[0].product_map_2+"' class='left' download><i><img src='/img/summary_icon03-2.png' alt=''></i><span>도면</span></a>";
                			inner += "<a class='left' onclick='actionSubmit(\""+result[0].product_map_2+"\",\"product_map\")'><i><img src='/img/summary_icon03-2.png' alt=''></i><span>도면</span></a>";
                			//inner += "<a href='/img/product_map/"+result[0].product_map_1+"' class='right' download><i><img src='/img/summary_icon04-2.png' alt=''></i></a>";
                			inner += "<a class='right' onclick='actionSubmit(\""+result[0].product_map_1+"\",\"product_map\")'><i><img src='/img/summary_icon04-2.png' alt=''></i></a>";
            			}
        				if (auth_chk!="" && auth_chk!=null) {
        					if (result[0].product_auth_file==null || result[0].product_auth_file=="" ) {
        						...
        						inner += "<a onclick='cert_actionSubmit(\""+result[0].product_auth_file+"\");'><i><img src='/img/summary_icon05.png' alt=''></i>인증서</a>";
    						}
        				}
					}
    			}
...
// 					}
					...
					if (chk_length<10) {
						...
						if (lang=='en') {
            				...
            				if (map_1_chk.indexOf('.dwg')!=-1 || map_1_chk.indexOf('.DWG')!=-1) {
                    			//inner += "<a href='/img/product_map/"+result[0].product_map_2+"' class='left' download><i><img src='/img/summary_icon03-2.png' alt=''></i><span>도면</span></a>";
                    			inner += "<a class='left' onclick='actionSubmit(\""+map_2_chk+"\",\"product_map\")'><i><img src='/img/summary_icon03-2.png' alt=''></i><span>도면</span></a>";
                    			//inner += "<a href='/img/product_map/"+result[0].product_map_1+"' class='right' download><i><img src='/img/summary_icon04-2.png' alt=''></i></a>";
                    			inner += "<a class='right' onclick='actionSubmit(\""+map_1_chk+"\",\"product_map\")'><i><img src='/img/summary_icon04-2.png' alt=''></i></a>";
                			}
        					if (auth_chk!="" && auth_chk!=null) {
        						...
        						if (result[i].product_auth_file==null || result[i].product_auth_file=="" ) {
            						...
            						inner += "<a onclick='cert_actionSubmit(\""+result[i].product_auth_file+"\");'><i><img src='/img/summary_icon05.png' alt=''></i>인증서</a>";
    							}
    						}
						}
					}
					...
					inner += "</div>";	
				//}
				if(result[i].product_title == "노출콘센트 3구 (삽입식)"){
    				console.log("========================삽입식================================");
    				console.log(map_1_chk);
    				console.log("========================================================");
				}
				if(result[i].product_title == "노출콘센트 3구 (조임식)"){
    				console.log("========================조임식================================");
    				console.log(map_1_chk);
    				console.log("========================================================");
				}
	...
	<div class="clearfix">
		...
		<?php 
		  if ($_GET['lang']=='en') {
		      ...
		      echo "<a href='/product-intro'>모든 제품군 보기</a>";
		  }
		?>
		</div>
		...
		<div class="tab tab02 cate_tab">
    		<?php 
    		  if ($_GET['lang']=='en') {
    		      ...
    		      echo "<div class='c_title cate1_title'>제품 구분 선택</div>";
    		  }
    		?>
		...
		<div class="tab tab03 cate_tab">
			<div class="c_title cate2_title">제품 항목 선택</div>
		...
		if ($_GET['lang']=='en') {
		    ...
		    echo "<div class='c_title cate3_title'>제품 항목 선택</div>";
		}
		?>
		...
		if ($row['product_indi']=='O' && strlen($row['product_info']) > 5) {
		 ...
		 <?php 
		    ...
		    if ($_GET['lang']=='en') {
    			...
    			echo "<p class='color-b'>*상세스펙을 보시려면 제품을 클릭하세요.</p>";
		    }
...
//                  }
	...
	if ($_GET['lang']=='en') {
	    ...
	    echo "<h1>카다로그</h1>";
	    echo "<p class='color-b'>*외장재에 대해 자세히 알고싶으면 카다로그를 참고해주세요.</p>";
	    echo "<div class='btn-wrap'>";
	    echo "<a onclick='actionSubmit(\"2021_KMEW_catalog_kr.pdf\",\"catalog\")'>KMEW 카다로그 다운로드</a>";
	    echo "<a href='https://www.kmew.co.jp/global/korea/' target='_blank' class='site'>일본 KMEW 사이트 (한국어)</a>";
	    echo "</div>";
	}
	?>
<!-- 		<h1>카다로그</h1> -->
...
<!-- 		<div class="btn-wrap"> -->
<!-- 		<a onclick='actionSubmit("2019 KMEW catalog.pdf","catalog")'>KMEW 카다로그 다운로드</a>  -->
<!-- 			<a href="https://www.kmew.co.jp/global/korea/" target="_blank" class="site">일본 KMEW 사이트 (한국어)</a> -->
<!-- 		</div> -->
	</section>
	<?php 	
	}
